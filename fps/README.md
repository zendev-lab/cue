...
FP 标题和正文使用中文，技术标识按需保留。PR 标题使用英文，正文复用仓库统一中文
模板。标题遵循 `zendev` profile，使用 `docs(fp)` scope，并以 `propose`、`revise`
或 `supersede` 表达本次文档变更；这些动词是评审惯例，不是机器状态。
