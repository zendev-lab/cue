...
```
┌─────────────────────────────────────────────────────────────┐
│ JOB  J:3 (1 running) A:1 C:2  cued:ok  mouse:ui  14:30 ... │ ← Split header
├────────────┬────────────────────────────────────────────────┤
│ Sidebar    │ ┌ stdout J1 × ───────────────────────────────┐ │
...
┌─────────────────────────────────────┐
│                                     │
│   (Job pty / Agent session view)    │
│                                     │
├─────────────────────────────────────┤
│  Job: Ctrl+Z detach                │
│  Agent: Enter send • Ctrl+C cancel │
└─────────────────────────────────────┘
```
...
| Job :fg 退出键 | Ctrl+Z |
| Agent :fg 控制 | Enter 发送 follow-up；Ctrl+C cancel 当前轮；Ctrl+Z 本地 detach |
...
- 选中后智能路由：
  ...
  - 已结束的 Job → 自动 `:out`
  - AGENT → 自动请求 `:fg A<n>`；前台视图与 session display 共用同一份会话内容
...
- 错误 → 红色文字
- `:ask` / `:spawn` / `:send A1 ...` → 下半只显示轻量 ack / status；真实 agent 对话内容进入 session view / `:fg A1`
...
| 补全 | Tab |
| 模式切换 | Shift+Tab 在任意非-FG 焦点循环 JOB→AGENT→CRON |
...
| Agent :fg | 打开统一前台会话视图；不再把 streamed/final 内容灌进普通结果卡片 |
