...
#[cfg(target_os = "linux")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_restart_refuses_unreachable_live_daemon_without_force_stopping_it() {
    run_daemon_test(async {
        ...
        assert!(!output.status.success(), "restart must fail closed");
        let stderr = String::from_utf8_lossy(&output.stderr);
        assert!(stderr.contains(&format!("cued pid {old_pid} is alive")));
        assert!(stderr.contains("no force-stop was attempted"));
        assert!(
            old_daemon.try_wait().expect("poll old daemon").is_none(),
            "a failed restart must not stop an unreachable live daemon"
        );
        ...
        timeout(Duration::from_secs(5), old_daemon.wait())
            .await
            .expect("old daemon did not exit during cleanup")
