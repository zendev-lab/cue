...
DROP TABLE IF EXISTS script_items;
...
/// closure. Durable execution, schedule, and named-session references are checked inside
...
pub fn sweep_scopes(conn: &Connection, reachable: &HashSet<ScopeHash>) -> Result<usize> {
        ...
        .query_row(
            "SELECT hex(scopes.hash)
             ...
             WHERE NOT EXISTS (
               ...
               AND (
                   scopes.hash IN (
                       SELECT current_scope FROM executions
                   )
                   OR scopes.hash IN (
                       SELECT scope_hash FROM schedules
                   )
                   OR scopes.hash IN (
    ...
    if let Some(hash) = unmarked_reference {
        return Err(anyhow!(
            "refusing scope sweep: persisted execution, schedule, or session still references unmarked scope {hash}"
        ));
    }
...
mod tests {
    ...
    fn migration_is_idempotent() {
        ...
        for table in [
            ...
            assert_eq!(count, 0, "retired table {table} must not survive v20");
        }
    }
