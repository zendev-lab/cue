...
/// memory while delegating durable state to the IPC v4 SQLite schema.
...
impl DaemonService {
    ...
    async fn drive(self: Arc<Self>, task: Arc<ExecutionTask>) -> Result<(), RuntimeError> {
        ...
        self.terminate_steps(&task, &to_cancel, cue_core::CancelMode::Force)
            .await;
        ...
        for ready in ready {
            ...
            tokio::spawn(async move {
                if let Err(error) = service.realize(task, ready).await {
                    tracing::error!(%error, "execution leaf failed");
                }
            });
        }
        Ok(())
        ...
        match &command {
            ...
            Command::Restart => {
                ...
                match self.service.store.record_durable_operation(
                    ...
                    OperationOutcome::Inserted => {
                        let _ = self.service.lifecycle.send(LifecycleSignal::Restart {
                            restart_id,
                            target_instance_id,
                        });
                        Ok(response)
            ...
            Command::Shutdown => {
                ...
                match self.service.store.record_durable_operation(
                    ...
                    OperationOutcome::Inserted => {
                        let _ = self.service.lifecycle.send(LifecycleSignal::Shutdown);
                        Ok(response)
                    }
                    OperationOutcome::Replay(response) => Ok(response),
                    OperationOutcome::Conflict => Err(operation_conflict()),
