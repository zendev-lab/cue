...
impl ExecutionClient {
    ...
    pub fn into_multiplexed(self) -> MultiplexedClient {
        MultiplexedClient::new(self)
    }
...
impl MultiplexedClient {
    fn new(client: ExecutionClient) -> Self {
        let ExecutionClient {
            client_id,
            stream,
            operation_prefix,
...
impl SurfaceTransport for ExecutionClient {
    async fn query(&mut self, query: Query) -> Result<ResultPayload> {
        ExecutionClient::query(self, query).await
    }
    async fn command(&mut self, command: Command) -> Result<ResultPayload> {
        ExecutionClient::command(self, command).await
    }
}

impl SurfaceTransport for &MultiplexedClient {
    async fn query(&mut self, query: Query) -> Result<ResultPayload> {
        MultiplexedClient::query(self, query).await
    }
    async fn command(&mut self, command: Command) -> Result<ResultPayload> {
        MultiplexedClient::command(self, command).await
    }
}
...
async fn dispatch_surface(
    ...
    let result = match command {
        ...
        SurfaceCommand::GetExecution { id } => client.query(Query::GetExecution { id }).await?,
        SurfaceCommand::WaitExecution { id } => client.query(Query::WaitExecution { id }).await?,
        ...
        SurfaceCommand::Frontend(action) => return Ok(SurfaceOutcome::Frontend(action)),
    };
...
mod tests {
    ...
    async fn multiplexed_surface_shares_read_only_tail_and_concurrent_wait_semantics() {
        ...
        let client = ExecutionClient::connect_stream(client_stream, client_id())
            .await
            .unwrap()
            .into_multiplexed();
        for source in [
        ...
        };
        {
            ...
            .unwrap();
            assert!(
                matches!(tokio::time::timeout(std::time::Duration::from_secs(3), wait).await.unwrap().unwrap(), ResultPayload::Execution { execution } if execution.state == cue_core::ExecutionState::Cancelled)
            );
        }
        drop(client);
    ...
    async fn surface_queries_do_not_store_scope_and_tail_reads_the_suffix() {
        ...
        let (client_stream, server_stream) = tokio::io::duplex(128 * 1024);
        let server = tokio::spawn(serve_stream(service, server_stream));
        let mut client = ExecutionClient::connect_stream(client_stream, client_id())
            .await
            .unwrap();
        client
    ...
    async fn reconnect_after_lost_submit_response_reuses_the_logical_operation() {
        ...
        let mut client = ExecutionClient::connect(&socket).await.unwrap();
