//! Cue surface syntax lowered into the closed execution algebra.
//!
...
use crate::surface::{ModeParams, ParamValue, PipeOp, command_prefers_foreground};
...
pub fn compile_command(
    ...
    mode: Mode,
    scope: ScopeHash,
) -> Result<SurfaceCommand, CompileError> {
    compile_resolved(parse_command(input, mode)?, scope)
}

pub fn compile_file(input: &str, scope: ScopeHash) -> Result<ExecutionSpec, CompileError> {
    match compile_resolved(parse_file_script_command(input)?, scope)? {
        SurfaceCommand::Submit(spec) => Ok(spec),
    ...
    match command {
        ...
        ResolvedCommand::Script { items } => {
            ...
            let plan = plans
                ...
                .ok_or_else(|| CompileError::Invalid("a .cue script is empty".into()))?;
        ...
        ResolvedCommand::Jobs | ResolvedCommand::Log { id: None } => {
            Ok(SurfaceCommand::ListExecutions)
        }
        ResolvedCommand::Log { id: Some(id) } => Ok(SurfaceCommand::GetExecution {
            id: parse_execution_id("log", &id)?,
        }),
        ResolvedCommand::Wait { id } => Ok(SurfaceCommand::WaitExecution {
            id: parse_execution_id("wait", &id)?,
        }),
        ResolvedCommand::Out { id, tail_bytes } => Ok(SurfaceCommand::ReadOutput {
            target: parse_output_target("out", &id)?,
            stream: OutputSelection::Stdout,
            tail_bytes,
        }),
        ResolvedCommand::Err { id } => Ok(SurfaceCommand::ReadOutput {
            target: parse_output_target("err", &id)?,
            stream: OutputSelection::Stderr,
            tail_bytes: None,
        }),
        ResolvedCommand::Cancel { id } => Ok(SurfaceCommand::CancelExecution {
            id: parse_execution_id("cancel", &id)?,
            force: false,
        }),
        ResolvedCommand::Kill { id } => Ok(SurfaceCommand::CancelExecution {
            id: parse_execution_id("kill", &id)?,
            force: true,
        }),
        ResolvedCommand::Fg { id, role } => Ok(SurfaceCommand::AttachPty {
        ...
        ResolvedCommand::Help { topic } => {
            Ok(SurfaceCommand::Frontend(FrontendAction::Help { topic }))
        }
        ResolvedCommand::Clear => Ok(SurfaceCommand::Frontend(FrontendAction::Clear)),
        ResolvedCommand::Quit => Ok(SurfaceCommand::Frontend(FrontendAction::Quit)),
        ResolvedCommand::Restart => Ok(SurfaceCommand::Frontend(FrontendAction::Restart)),
        ResolvedCommand::Cron {
            schedule,
            chain,
            params,
        } => {
            drop((schedule, chain, params));
        ...
        ResolvedCommand::Crons => Err(CompileError::ExternalOwner {
            feature: "schedule",
            owner: "a scheduler must submit ordinary ExecutionSpec values",
        }),
        ...
        ResolvedCommand::Scopes => Err(CompileError::ExternalOwner {
            feature: "scope history",
            owner: "clients may store explicit Scope values but the kernel has no scope cursor",
        }),
...
fn execution(scope: ScopeHash, plan: ExecutionPlan) -> Result<SurfaceCommand, CompileError> {
    Ok(SurfaceCommand::Submit(
        ExecutionSpec::new(scope, plan).map_err(invalid)?,
    ))
}

fn compile_script_item(command: ResolvedCommand) -> Result<ExecutionPlan, CompileError> {
    match command {
        ResolvedCommand::Run { chain, params } => {
            let io = validate_run_params(&params)?;
...
fn validate_run_params(params: &ModeParams) -> Result<Option<IoMode>, CompileError> {
        ...
        if name != "pty" {
            return Err(CompileError::ExternalOwner {
                ...
                owner: match name.as_str() {
                    ...
                    "scope" => "the kernel threads Scope only inside an Execution",
    ...
    match params.get("pty") {
        ...
        Some(ParamValue::Str(_)) => {
            Err(CompileError::Invalid("pty expects a boolean value".into()))
        }
    }
}
...
fn compile_chain(
    ...
    match node {
        ...
        ChainNode::Parallel { left, op, right } => ExecutionPlan::parallel(
            ...
            match op {
                ParallelOp::All => cue_core::ParallelJoin::All,
                ParallelOp::Race => cue_core::ParallelJoin::AnySuccess,
            },
        )
        .map_err(invalid),
    }
...
fn compile_pipeline(pipeline: AstPipeline) -> Result<Pipeline, CompileError> {
    let mut segments = pipeline.segments.into_iter();
    let first = segments
        .next()
        .ok_or_else(|| CompileError::Invalid("pipeline must contain a process".into()))?;
    ...
    for segment in segments {
        let link = previous_link.ok_or_else(|| {
            CompileError::Invalid("pipeline is missing a link between processes".into())
        })?;
...
fn compile_builtin(words: &[String]) -> Result<Option<ExecutionPlan>, CompileError> {
...
fn builtin_cd(words: &[String]) -> Result<ExecutionPlan, CompileError> {
...
fn builtin_env(words: &[String]) -> Result<ExecutionPlan, CompileError> {
...
fn builtin_umask(words: &[String]) -> Result<ExecutionPlan, CompileError> {
    ...
    let mask = u16::from_str_radix(mask.trim_start_matches("0o"), 8)
        .map_err(|_| CompileError::Invalid(format!("invalid octal umask `{mask}`")))?;
    Ok(ExecutionPlan::builtin(BuiltinCommand::umask(
        FileModeMask::new(mask).map_err(invalid)?,
    )))
}

fn surface_env_key(key: &str) -> Result<EnvKey, CompileError> {
...
fn parse_output_target(command: &str, input: &str) -> Result<OutputTarget, CompileError> {
    if input.contains('/') {
        input
            .parse::<StepId>()
            .map(OutputTarget::Step)
            .map_err(|_| {
                CompileError::Invalid(format!("`:{command}` expects E1 or E1/S1, got `{input}`"))
            })
    } else {
...
fn invalid(error: impl std::fmt::Display) -> CompileError {
    CompileError::Invalid(error.to_string())
}

#[cfg(test)]
mod tests {
    use cue_core::{BuiltinCommand, EnvEdit, ExecutionPlan, IoMode, SequenceCondition};
    ...
    fn plan(input: &str) -> ExecutionPlan {
        match compile_command(input, Mode::Job, SCOPE).unwrap() {
            SurfaceCommand::Submit(spec) => spec.plan().clone(),
    ...
    fn surface_environment_names_remain_shell_like() {
        let error = compile_command(":env set A-B=value", Mode::Job, SCOPE).unwrap_err();
        assert!(error.to_string().contains("invalid surface environment"));
    }

    #[test]
    fn builtins_cannot_be_disguised_as_pipeline_processes() {
        let error = compile_command("cd repo |> pwd", Mode::Job, SCOPE).unwrap_err();
        assert!(error.to_string().contains("cannot be pipeline processes"));

        let error = compile_command("A=one cd repo", Mode::Job, SCOPE).unwrap_err();
        assert!(
            error
                .to_string()
